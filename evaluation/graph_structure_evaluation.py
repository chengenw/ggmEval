...
class Descriptor():
    ...
    def disc(self, samples1, samples2, **kwargs):
        ...
        if not self.is_parallel:
            for s1 in tqdm(samples1, desc='disc() series'):
                for s2 in samples2:
                    tot_dist += [self.distance(s1, s2)]
        else:
            with concurrent.futures.ProcessPoolExecutor(max_workers=self.max_workers) as executor:
                for dist in tqdm(executor.map(self.kernel_parallel_worker,
                        [(s1, samples2, partial(self.distance, **kwargs)) for s1 in samples1]), desc='disc() parallel'):
                    tot_dist += [dist]
    ...
    def extract_features(self, dataset):
        ...
        if self.is_parallel:
            with concurrent.futures.ThreadPoolExecutor(max_workers=self.max_workers) as executor:
                for spectral_density in tqdm(executor.map(self.spectral_worker, dataset), desc='Spectral parallel'):
                    res.append(spectral_density)
        else:
            for g in tqdm(dataset, desc='Spectral series'):
