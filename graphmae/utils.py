...
def dropout_edge(edge_index: Tensor, p: float = 0.5,
    ...
    if force_undirected:
        ...
        keep_mask = edge_mask.nonzero().squeeze()
        drop_mask = drop_mask.nonzero().squeeze()

    return edge_index, keep_mask, drop_mask
...
def build_args():
    ...
    cprint('@@@parse known args for GraphMAE', 'red')
...
def load_best_configs(args, path):
    ...
    if dataset_name not in configs:
        cprint(f'\n{dataset_name}: Best args not found\n', 'red')
        logging.info(f"{dataset_name}: Best args not found")
        return args

    cprint('\n***load best configs\n', 'red')
    logging.info(f"{dataset_name}: Using best configs")
