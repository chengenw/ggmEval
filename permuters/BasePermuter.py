...
class BasePermuter():
    ...
    def make_er_graphs(self, args, reference_set):
        ...
        print(f'creating {len(reference_set)} ER graphs...')
        # Generate ER graphs
        # generated_graphs = [
        ...
        for g, p in zip(reference_set, avg_p):
            while True:
                ...
                graph = nx.erdos_renyi_graph(g.number_of_nodes(), p, seed=seed)
                if graph.number_of_edges() == 0:
                    # print(f'n {g.number_of_nodes()}, p {p:.4f}, seed {seed}')
                    continue
                break
            generated_graphs.append(graph)

        print(f'done creating {len(generated_graphs)} ER graphs. time elapsed: {(time.time() - start_time)/60:.4f} minutes')

        generated_graphs = [dgl.DGLGraph(g) for g in generated_graphs]
        generated_graphs = [g.to(args.device) for g in generated_graphs]

        # if args.dataset in ['zinc']:
        if args.dataset in ['zinc'] or args.dataset in args.dataset_names or re.match(r'(mnist|pattern|cifar10|tsp)(_\d+k)?', args.dataset):
        # if args.dataset in ['zinc'] or args.node_label and (args.dataset in args.dataset_names or re.match(r'(mnist|pattern|cifar10|tsp)(_\d+k)?', args.dataset)):  # Randomly assign node/edge labels
