...
class GNNEncoder(nn.Module):
    ...
    def get_embed(self, loader, device='cpu', pooler='sum'):
        embed_arr = []
        for data in loader:
            ...
            x = self.activation(x)
            if pooler == 'sum':
                embed = global_add_pool(x, data.batch)
            embed_arr.append(embed)
        embed_g = torch.cat(embed_arr, dim=0)
        return embed_g
    ...
    @torch.no_grad()
    def embed(self, x, edge_index, batch, l2_normalize=False, pooling='sum'):
...
class MaskGAE(nn.Module):
    ...
    def get_embed(self, loader, device):
        return self.encoder.get_embed(loader, device)

    def get_graph_embed(self, x, edge_index, batch, edge_attr=None):
        self.eval()
        out = self.encoder.embed(x, edge_index, batch)

        return out

    # def forward_loader(self, loader, device='cpu'):
    #     embed_arr = []
    #     for data in loader:
    #         embed = self.encoder(data.x, data.edge_index.to(device))
    #         embed_arr.append(embed)
    #     embed_arr = torch.cat(embed_arr, dim=0)
    #
    #     return embed_arr
    ...
    def train_epoch_loader(
        self, data_loader, optimizer, alpha=0.002,
        edge_batch_size=2 ** 16, grad_norm=1.0, device='cpu'
    ):
        loss_total = 0.0
        total_edges = 0
        # for data in tqdm(data_loader, desc='train_epoch_loader'):
        for data in data_loader:
            ...
            for perm in DataLoader(
                range(masked_edges.size(1)), batch_size=edge_batch_size, shuffle=True
            ):
                # cprint(f'edge batch size is {None if type(perm) is int else len(perm)}', 'red')
                optimizer.zero_grad()
            ...
            total_edges += masked_edges.size(1)

        return loss_total / total_edges
    ...
    @torch.no_grad()
    def test_loader(self, train_loader, test_loader, device='cpu'):
        y_arr, pred_arr = [], []
        for train_data, test_data in zip(train_loader, test_loader):
            z = self.encoder(train_data.x.to(device), train_data.edge_index.to(device))
            pos_edge_index, neg_edge_index = test_data.pos_edge_label_index, test_data.neg_edge_label_index
        ...
        yy = np.concatenate(y_arr)
        pp = np.concatenate(pred_arr)

        return roc_auc_score(yy, pp), average_precision_score(yy, pp)
