...
def eval_graphcl(encoder_model, contrast_model, dataloader, device=None):
    encoder_model.eval()
    epoch_loss = 0
    with torch.no_grad():
...
def train_graphcl(encoder_model, contrast_model, dataloader, optimizer, device=None):
    encoder_model.train()
    epoch_loss = 0
    for data in dataloader:
        optimizer.zero_grad()
...
def get_graphcl_model(dataset, args):
    ...
    with tqdm(total=args.epochs, desc='graphcl: train') as pbar:
        for epoch in range(1, args.epochs+1):
            loss = train_graphcl(encoder_model, contrast_model, dataloader, optimizer, args.device)
            args.writer.add_scalar('loss', loss / (len(dataset)/dataloader.batch_size + 1), epoch)
            pbar.set_postfix({'loss': loss})
            pbar.update()

    print('evaluating the loss: ')
    eval_losses = []
    with tqdm(total=10, desc='graphcl: eval') as pbar:
        for epoch in range(10):
            eval_losses.append(eval_graphcl(encoder_model, contrast_model, dataloader, args.device))
            pbar.set_postfix({'loss': loss})
            pbar.update()
...
def get_model(dataset, args, labels=None):
    path = f'logs'
    if not os.path.exists(path):
        os.mkdir(path)

    path = f'{path}/{args.feature_extractor}'
    if not os.path.exists(path):
        os.mkdir(path)
    s_feat = 'no_'
    if args.deg_feats:
    ...
    if args.clus_feats:
        s_feat = 'clus_'
    args.writer = SummaryWriter(f'{path}/{args.dataset}_{s_feat}{strftime("%Y-%m-%d %H:%M:%S")}')

    if not os.path.exists('saved_models'):
        os.makedirs('saved_models')

    if not isinstance(dataset[0], Data):
        pyg_dataset = make_dataset_ready_to_save(copy.deepcopy(dataset), parallel=args.is_parallel) # dgl to pyg
    ...
    if args.feature_extractor == 'graphcl':
        ...
        torch.save(gconv, f'saved_models/{args.model_name}')
        return gconv, None
    elif args.feature_extractor == 'graphmae':
        from main_graph import train_mae
        train_mae(args, pyg_dataset, labels)
    elif args.feature_extractor == 'maskgae':
        from train_nodeclas import train_maskgae
        train_maskgae(args, pyg_dataset)
    else:
        # raise ValueError('model name not found')
        print(f'feature extractor is {args.feature_extractor}')